...
export default class entriesController{
    ...
    static async apiGetResults(req, res, next){
        ...
        const entriesPerPage = req.query.entriesPerPage ? parseInt(req.query.entriesPerPage) : null;
        ...
        let filters = {};
        //Was there a text field? If so, we are executing a general search.
        if(req.query.text){
            filters.text = req.query.text;
        }
        else {
            //Was there a title field given in the search query? If so, then our search will ask for it.
            if(req.query.title){
                filters.title = req.query.title;
            }
            //Was there a subtitle field given in te searc query? If so, then our search will ask for it.
            if(req.query.subtitle){
                filters.subtitle = req.query.subtitle;
            }
            //...
            if(req.query.src){
            ...
            if(req.query.keywords){
                filters.keywords = req.query.keywords;
            }
            if(req.query.authors){
                filters.authors = req.query.authors;
            }
            if(req.query.hid){
        ...
        let response = {
            ...
            page: page,
            filters: filters,
            entriesPerPage: contents[2] == 50? "non-Given" : contents[2],
        }
